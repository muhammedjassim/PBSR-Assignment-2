...
```{r, include=FALSE}
knitr::opts_chunk$set(echo=FALSE,warning=FALSE, message = FALSE, fig.height = 5, fig.width = 10, fig.align = "center")
```
...
\begin{eqnarray*}
\mathbb{P}(\text{atleast one goal}) & = & 1 - \mathbb{P}(\text{no goal})\\
                                    & = & 1 - (1-r)\\
                                    & = & r\\
                                    & = & 0.6\\
\end{eqnarray*}
...
\begin{eqnarray*}
\mathbb{P}(1<=X<4) & = & \sum_{i=1}^{3} \mathbb{P}(X=i)\\
                   & = & \sum_{i=1}^{3} (1-r)r^i\\
                   & = & r(1-r^3)\\
                   & = & 0.47\\
\end{eqnarray*}
...
\begin{itemize}
\item Poisson has the least variance and also seems to have central tendencies closer to the expected values, hence it could be a better fit.
\end{itemize}
